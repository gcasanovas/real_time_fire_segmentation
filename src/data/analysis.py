...
def plot_single_mask(mask: np.ndarray, 
                     title: Optional[str] = None,
                     resize_shape: Optional[Tuple[int, int]] = None) -> None:
    """
    Plot a single mask from the dataset.

    Parameters
    ----------
    mask : np.ndarray
        The segmentation mask of the given image.
    title : str, optional
        The title of the plot. If not provided it is assigned
        manually. By default None.
    resize_shape : (int, int), optional
        The size used to reshape the mask before plotting.
        By default None.
    """
    ...
    # Create a subplot for the single mask
    plt.figure(figsize=(6, 6))
    plt.imshow(mask, cmap='gray', vmin=0., vmax=1.)
    plt.axis('off')

    # Add a legend
    legend_elements = [Patch(facecolor='w', edgecolor='black', label='Fire mask')]
    plt.legend(handles=legend_elements, loc='upper left')

    if title is None:
        title = 'Representation of a single mask from the dataset'
