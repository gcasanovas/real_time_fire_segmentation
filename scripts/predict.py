...
from src.prediction.analysis import plot_image_prediction,save_predicted_mask
...
def main():
    ...
    parser.add_argument(
        '--save-path', '-s', metavar='Save Path', type=str, default=None,
        help='Path to save the output plot (image and predicted mask). If not '
             'provided, the plot will not be saved.', required=False)
    ...
    if save_path:
        ...
        print('Save image')
        save_predicted_mask(predicted_mask, resize_shape=ORIGINAL_SHAPE, save_path=save_path)
    else:
        print('Starting plotting...')
        # Just plot if no save path is provided.
